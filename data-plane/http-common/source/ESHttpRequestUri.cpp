...
HttpRequestUri::HttpRequestUri()
    : _type(ES_HTTP_URI_HTTP), _port(-1), _host(NULL), _absPath(NULL), _query(NULL), _fragment(NULL), _other(NULL) {}

HttpRequestUri::~HttpRequestUri() {}

void HttpRequestUri::reset() {
  _type = ES_HTTP_URI_HTTP;
  _port = -1;
  _host = NULL;
  _absPath = NULL;
  _query = NULL;
...
int HttpRequestUri::Compare(const HttpRequestUri *r1, const HttpRequestUri *r2) {
  if (ES_HTTP_URI_ASTERISK == r1->type() && ES_HTTP_URI_ASTERISK == r2->type()) {
    return 0;
  }

  if (ES_HTTP_URI_OTHER == r1->type() && ES_HTTP_URI_OTHER == r2->type()) {
  ...
  if (r1->type() != r2->type()) {
    return ES_HTTP_URI_HTTP == r1->type() ? 1 : -1;
  }
  ...
  int port1 = 0 <= r1->port() ? r1->port() : (ES_HTTP_URI_HTTP == r1->type() ? 80 : 443);
  int port2 = 0 <= r2->port() ? r2->port() : (ES_HTTP_URI_HTTP == r2->type() ? 80 : 443);
