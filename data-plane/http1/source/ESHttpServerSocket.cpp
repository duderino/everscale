...
ESB::Error HttpServerSocket::sendResponse(const HttpResponse &response, es_http_header_filter filter, void *context) {
  ESB_LOG_DEBUG("[%s] sending response %d %s", _socket->name(), response.statusCode(), response.reasonPhrase());

  ESB::Error error = _transaction->response().copy(&response, _transaction->allocator(), filter, context);
  if (ESB_SUCCESS != error) {
    return error;
  }
