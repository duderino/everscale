...
/** A dispatcher that evenly distributes sockets across multiple socket multiplexers.
 *
 * @ingroup network
 */
...
public:
     ...
     *  the multiplexers.  Each multiplexer will handle maximumSockets/multiplexerCount
     *  sockets.
     ...
     */
    ESFSocketMultiplexerDispatcher(ESFUInt16 maximumSockets, ESFUInt16 multiplexerCount,
     ...
     *  will add the socket to the multiplexer handling the fewest number of sockets.
     *
     ...
     *
     * @param multiplexerIndex The index of the multiplexer.  This ranges from 0 to
     *  multiplexerCount - 1, inclusive of endpoints.
     * @param multiplexedSocket The multiplexed socket to add
     * @return ESF_SUCCESS if successful, ESF_OVERFLOW if the maximum sockets this
     *  multiplexer can handle has been reached, another error code otherwise.
     */
    ESFError addMultiplexedSocket(int multiplexerIndex, ESFMultiplexedSocket *multiplexedSocket);
...
private:
    ...
    ESFSocketMultiplexerDispatcher &operator=(const ESFSocketMultiplexerDispatcher &);
