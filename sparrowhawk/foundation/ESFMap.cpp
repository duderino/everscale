...
ESFMapNode::ESFMapNode(ESFMapNode *parent, ESFMapNode *left, ESFMapNode *right, bool isBlack, const void *key,
        void *value) :
...
ESFMap::ESFMap(bool isUnique, ESFComparator *comparator, ESFAllocator *allocator, ESFLockable *lockable) :
    _size(0), _isUnique(isUnique), _root(&_sentinel), _allocator(allocator), _lockable(lockable), _comparator(
            comparator), _sentinel(&_sentinel, &_sentinel, &_sentinel, true, 0, 0)
...
ESFError ESFMap::insert(const void *key, void *value) {
    ...
    ESFMapNode *node = new (_allocator) ESFMapNode(&_sentinel, &_sentinel, &_sentinel, true, key, value);
...
ESFError ESFMap::find(const void *key, void **value) {
    if (!key || !value) {
        return ESF_NULL_POINTER;
    }
...
ESFError ESFMap::find(const void *key, ESFMapIterator *iterator) {
    if (!key || !iterator) {
        return ESF_NULL_POINTER;
    }
