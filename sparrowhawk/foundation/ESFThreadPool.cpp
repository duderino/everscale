...
public:
    ESFThreadPoolWorker(int workerId, const char *name, ESFSharedEmbeddedQueue *queue, ESFLogger *logger);
...
ESFThreadPool::ESFThreadPool(const char *name, int threads, ESFLogger *logger, ESFAllocator *allocator) :
    _numThreads(threads < MIN_THREADS ? MIN_THREADS : threads), _name(name ? name : "ThreadPool"), _threads(0),
            _allocator(allocator ? allocator : ESFSystemAllocator::GetInstance()), _logger(logger ? logger
                    : ESFNullLogger::GetInstance()), _queue() {
}
...
ESFError ESFThreadPool::start() {
    ...
    if (false == createWorkerThreads()) {
        if (_logger->isLoggable(ESFLogger::Critical)) {
            _logger->log(ESFLogger::Critical, __FILE__, __LINE__, "[%s] cannot start: Out Of Memory");
        }
    ...
    for (int i = 0; i < _numThreads; ++i) {
        if (_logger->isLoggable(ESFLogger::Notice)) {
            _logger->log(ESFLogger::Notice, __FILE__, __LINE__, "[%s] starting worker %d", _name, i + 1);
        }
        ...
        if (ESF_SUCCESS != error) {
            if (_logger->isLoggable(ESFLogger::Critical)) {
                ...
                _logger->log(ESFLogger::Critical, __FILE__, __LINE__, "[%s] cannot start worker %d: %s", _name, i + 1,
                        buffer);
            }
...
void ESFThreadPool::stop() {
    ...
    // worker threads will get ESF_SHUTDOWN next time they try to pull an item from the queue
    _queue.stop();
    ...
    for (int i = 0; i < _numThreads; ++i) {
        if (_logger->isLoggable(ESFLogger::Debug)) {
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[%s] waiting for worker %d", _name, i + 1);
        }
        ...
        if (ESF_SUCCESS != error) {
            if (_logger->isLoggable(ESFLogger::Warning)) {
                ...
                _logger->log(ESFLogger::Warning, __FILE__, __LINE__, "[%s] error joining worker %d: %s", _name, i + 1,
                        buffer);
            }
        }
    }
...
bool ESFThreadPool::createWorkerThreads() {
    _threads = (ESFThread **) _allocator->allocate(_numThreads * sizeof(ESFThread *));
    ...
    for (int i = 0; i < _numThreads; ++i) {
        _threads[i] = new (_allocator) ESFThreadPoolWorker(i + 1, _name, &_queue, _logger);
...
ESFThreadPoolWorker::ESFThreadPoolWorker(int workerId, const char *name, ESFSharedEmbeddedQueue *queue,
        ESFLogger *logger) :
...
void ESFThreadPoolWorker::run() {
    if (_logger->isLoggable(ESFLogger::Notice)) {
        _logger->log(ESFLogger::Notice, __FILE__, __LINE__, "[%s:%d] starting", _name, _workerId);
    }
    ...
    while (isRunning() && errorCount < 10) {
        ...
        if (ESF_SHUTDOWN == error) {
            if (_logger->isLoggable(ESFLogger::Debug)) {
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[%s:%d] received shutdown command", _name,
                        _workerId);
            }
        ...
        if (ESF_SUCCESS != error) {
            if (_logger->isLoggable(ESFLogger::Error)) {
                ...
                _logger->log(ESFLogger::Error, __FILE__, __LINE__, "[%s:%d] cannot pop command from queue: %s", _name,
                        _workerId, buffer);
            }
        ...
        if (_logger->isLoggable(ESFLogger::Debug)) {
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[%s:%d] starting command %s", _name, _workerId,
                    command->getName());
        }
        ...
        if (_logger->isLoggable(ESFLogger::Debug)) {
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[%s:%d] finished command %s", _name, _workerId,
                    command->getName());
        }
    ...
    if (_logger->isLoggable(ESFLogger::Notice)) {
        _logger->log(ESFLogger::Notice, __FILE__, __LINE__, "[%s:%d] exiting", _name, _workerId);
    }
}
