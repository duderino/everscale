...
ESFError ESFGetLastError() {
    return ESFConvertError(errno);
}
...
ESFError ESFGetLastError()
{
    return ESFConvertError( GetLastError() );
}
...
static char *ErrorDescriptions[] = { "Success", "Other error", "Operation not supported", "Null pointer",
        ...
        "Hard limit reached", "Cannot convert", "Illegal Encoding", "Unsupported Charset", "Index out of bounds",
        "Shutdown in progress", "Partial success" };
