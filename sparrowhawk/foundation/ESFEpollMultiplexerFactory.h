...
public:
    ...
    /** Constructor.
     *
     * @param name The name of the created multiplexer to be used in log messages.  Caller is
     *  responsible for the strings memory - use a string literal if possible.
     * @param logger An optional logger.  Pass NULL to not log anything.
     * @param allocator epoll socket multiplexers will be allocated using this allocator.
     */
    ESFEpollMultiplexerFactory(const char *name, ESFLogger *logger, ESFAllocator *allocator);
