...
public:
     ...
     */
    inline SOCKET getSocketDescriptor() const {
        return _sockFd;
    }
     ...
     */
    inline ESFError getLastError() {
        return GetLastError(_sockFd);
    }

    /** Get and clear the last error on a socket descriptor.
     *
     *  @param socket The socked descriptor
     *  @return the last error that occurred on the socket descriptor or ESF_SUCCESS if
     *      no error has occurred.
     */
