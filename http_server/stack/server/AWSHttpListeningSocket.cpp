...
AWSHttpListeningSocket::AWSHttpListeningSocket(AWSHttpServerHandler *handler,
                                               ESFListeningTCPSocket *socket,
...
bool AWSHttpListeningSocket::wantConnect()
{
    return false;
}

bool AWSHttpListeningSocket::wantRead()
{
    return false;
}

bool AWSHttpListeningSocket::wantWrite()
{
    return false;
}

bool AWSHttpListeningSocket::isIdle()
{
    return false;
}

bool AWSHttpListeningSocket::handleAcceptEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(_socket);
    ...
    AWSHttpServerHandler::Result result = _handler->acceptConnection(&acceptData._peerAddress);
    ...
    if (ESF_SHUTDOWN == error)
    {
        if (_logger->isLoggable(ESFLogger::Warning))
        {
            _logger->log(ESFLogger::Warning, __FILE__, __LINE__,
                         "[listener:%d] Dispatcher has been shutdown, closing newly accepted connection",
                         _socket->getSocketDescriptor());
        }
    ...
    if (ESF_SUCCESS != error)
    {
        if (_logger->isLoggable(ESFLogger::Error))
        {
            ...
            _logger->log(ESFLogger::Error, __FILE__, __LINE__,
                         "[listener:%d] Error adding newly accepted connection to the dispatcher: %s",
                         _socket->getSocketDescriptor(), buffer);
        }
...
bool AWSHttpListeningSocket::handleConnectEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        _logger->log(ESFLogger::Error, __FILE__, __LINE__,
...
bool AWSHttpListeningSocket::handleReadableEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        _logger->log(ESFLogger::Error, __FILE__, __LINE__,
...
bool AWSHttpListeningSocket::handleWritableEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        _logger->log(ESFLogger::Error, __FILE__, __LINE__,
...
bool AWSHttpListeningSocket::handleErrorEvent(ESFError errorCode, ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        char buffer[100];
...
bool AWSHttpListeningSocket::handleEndOfFileEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        _logger->log(ESFLogger::Error, __FILE__, __LINE__,
...
bool AWSHttpListeningSocket::handleIdleEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Error))
    {
        _logger->log(ESFLogger::Error, __FILE__, __LINE__,
...
bool AWSHttpListeningSocket::handleRemoveEvent(ESFFlag *flag, ESFLogger *logger)
{
