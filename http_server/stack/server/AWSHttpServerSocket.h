...
#define AWS_HTTP_SERVER_SOCKET_H
 ...
 */
class AWSHttpServerSocket : public ESFMultiplexedSocket
{
public:
     ...
     */
    AWSHttpServerSocket(AWSHttpServerHandler *handler,
     ...
     */
    ESFError reset(AWSHttpServerHandler *handler, ESFTCPSocket::AcceptData *acceptData);
...
private:
    // Disabled
    AWSHttpServerSocket(const AWSHttpServerSocket &);
    AWSHttpServerSocket &operator=(const AWSHttpServerSocket &);

    ESFError parseRequestHeaders(ESFFlag *isRunning, ESFLogger *logger);

    ESFError parseRequestBody(ESFFlag *isRunning, ESFLogger *logger);

    ESFError skipTrailer(ESFFlag *isRunning, ESFLogger *logger);

    ESFError formatResponseHeaders(ESFFlag *isRunning, ESFLogger *logger);

    ESFError formatResponseBody(ESFFlag *isRunning, ESFLogger *logger);

    ESFError flushBuffer(ESFFlag *isRunning, ESFLogger *logger);

    bool sendResponse(ESFFlag *isRunning, ESFLogger *logger);

    bool sendBadRequestResponse(ESFFlag *isRunning, ESFLogger *logger);

    bool sendInternalServerErrorResponse(ESFFlag *isRunning, ESFLogger *logger);
    ...
    AWSHttpServerHandler *_handler;
    AWSHttpServerCounters *_counters;

    AWSHttpServerTransaction _transaction;
    ESFConnectedTCPSocket _socket;
};

#endif /* ! AWS_HTTP_SERVER_SOCKET_H */
