...
AWSHttpServerSocketFactory::AWSHttpServerSocketFactory(AWSHttpServerCounters *counters,
                                                       ESFLogger *logger) :
...
{
    return _allocator.initialize(ESF_WORD_ALIGN(sizeof(AWSHttpServerSocket)) * 1000,
                                 ESFSystemAllocator::GetInstance());
}

void AWSHttpServerSocketFactory::destroy()
{
    AWSHttpServerSocket *socket = (AWSHttpServerSocket *) _embeddedList.removeFirst();
...
AWSHttpServerSocket *AWSHttpServerSocketFactory::create(AWSHttpServerHandler *handler,
                                                        ESFTCPSocket::AcceptData *acceptData)
{
    ...
    if (0 == socket)
    {
        socket = new(&_allocator) AWSHttpServerSocket(handler,
                                                      &_cleanupHandler,
                                                      _logger,
                                                      _counters);
...
AWSHttpServerSocketFactory::CleanupHandler::CleanupHandler(AWSHttpServerSocketFactory *factory) :
    ESFCleanupHandler(),
    _factory(factory)
{
}
