...
ESFError AWSHttpServerSocket::reset(AWSHttpServerHandler *handler, ESFTCPSocket::AcceptData *acceptData)
{
...
bool AWSHttpServerSocket::wantAccept()
{
    return false;
}

bool AWSHttpServerSocket::wantConnect()
{
    return false;
}
...
{
    return _state & (TRANSACTION_BEGIN | PARSING_HEADERS | PARSING_BODY | SKIPPING_TRAILER) ? true : false;
}

bool AWSHttpServerSocket::wantWrite()
{
    return _state & (FORMATTING_HEADERS | FLUSHING_HEADERS | FORMATTING_BODY | FLUSHING_BODY) ? true : false;
}
...
bool AWSHttpServerSocket::handleAcceptEvent(ESFFlag *isRunning, ESFLogger *logger)
{
...
bool AWSHttpServerSocket::handleConnectEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpServerSocket::handleReadableEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ...
    ESF_ASSERT(_state & (TRANSACTION_BEGIN | PARSING_HEADERS | PARSING_BODY | SKIPPING_TRAILER));
    ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));

    if (_state & TRANSACTION_BEGIN)
    {
        // If we send a response before fully reading the request, we can't reuse the socket
    ...
    while (isRunning->get())
    {
        ...
        // server handler should have populated the response object in parseRequestBody()
...
bool AWSHttpServerSocket::handleWritableEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ...
    while (isRunning->get())
    {
        ...
        _handler->end(&_transaction, AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_END);
        ...
        if (CloseAfterErrorResponse && 300 <= _transaction.getResponse()->getStatusCode())
        {
            return false;   // remove from multiplexer
        }
        ...
        if (_transaction.getRequest()->getReuseConnection())
        {
            ...
            return true;    // keep in multiplexer; but always yield after a http transaction
        }
        else
        {
            return false;   // remove from multiplexer
...
bool AWSHttpServerSocket::handleErrorEvent(ESFError errorCode, ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpServerSocket::handleEndOfFileEvent(ESFFlag *isRunning, ESFLogger *logger)
{
...
bool AWSHttpServerSocket::handleIdleEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpServerSocket::handleRemoveEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
    ...
    {
        _handler->end(&_transaction, AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_RECV_REQUEST_HEADERS);
    }
    else if (_state & PARSING_BODY)
    {
        _handler->end(&_transaction, AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_RECV_REQUEST_BODY);
    }
    else if (_state & (FORMATTING_HEADERS|FLUSHING_HEADERS))
    {
        _handler->end(&_transaction, AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_SEND_RESPONSE_HEADERS);
    }
    else if (_state & (FORMATTING_BODY|FLUSHING_BODY))
    {
        _handler->end(&_transaction, AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_SEND_RESPONSE_BODY);
    }
...
ESFError AWSHttpServerSocket::parseRequestHeaders(ESFFlag *isRunning, ESFLogger *logger)
{
    ...
    if (_logger->isLoggable(ESFLogger::Debug))
    {
        ...
        _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                     "[socket:%d] Method: %s",
                     _socket.getSocketDescriptor(), _transaction.getRequest()->getMethod());
        ...
        switch (_transaction.getRequest()->getRequestUri()->getType())
        {
            ...
            case AWSHttpRequestUri::AWS_URI_HTTPS:
                ...
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Scheme: %s",
                             _socket.getSocketDescriptor(),
                             AWSHttpRequestUri::AWS_URI_HTTP == _transaction.getRequest()->getRequestUri()->getType() ? "http" : "https");
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Host: %s",
                             _socket.getSocketDescriptor(),
                             0 == _transaction.getRequest()->getRequestUri()->getHost() ? "none" : (const char *) _transaction.getRequest()->getRequestUri()->getHost());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Port: %d",
                             _socket.getSocketDescriptor(), _transaction.getRequest()->getRequestUri()->getPort());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] AbsPath: %s",
                             _socket.getSocketDescriptor(), _transaction.getRequest()->getRequestUri()->getAbsPath());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Query: %s",
                             _socket.getSocketDescriptor(),
                             0 == _transaction.getRequest()->getRequestUri()->getQuery() ? "none" : (const char *) _transaction.getRequest()->getRequestUri()->getQuery());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Fragment: %s",
                             _socket.getSocketDescriptor(),
                             0 == _transaction.getRequest()->getRequestUri()->getFragment() ? "none" : (const char *) _transaction.getRequest()->getRequestUri()->getFragment());
            ...
            case AWSHttpRequestUri::AWS_URI_OTHER:
                ...
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] Other: %s",
                             _socket.getSocketDescriptor(), _transaction.getRequest()->getRequestUri()->getOther());
        ...
        for (AWSHttpHeader *header = (AWSHttpHeader *) _transaction.getRequest()->getHeaders()->getFirst();
             header;
...
ESFError AWSHttpServerSocket::parseRequestBody(ESFFlag *isRunning, ESFLogger *logger)
{
    ...
    while (isRunning->get())
    {
        ESFError error = _transaction.getParser()->parseBody(_transaction.getIOBuffer(),
                                                              &startingPosition,
                                                              &chunkSize);
        ...
        if (0 == chunkSize)
        {
            ...
            AWSHttpServerHandler::Result result = _handler->receiveRequestBody(&_transaction, &byte, 0);
            ...
            if (AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_CLOSE == result)
            {
                ...
                {
                    _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                                 "[socket:%d] server handler aborting connection after last body chunk",
                                 _socket.getSocketDescriptor());
                }
        ...
        AWSHttpServerHandler::Result result = _handler->receiveRequestBody(&_transaction,
        ...
        switch (result)
        {
            case AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_CLOSE:
                ...
                {
                    _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                                 "[socket:%d] server handler aborting connection before last body chunk",
                                 _socket.getSocketDescriptor());
                }
            ...
            case AWSHttpServerHandler::AWS_HTTP_SERVER_HANDLER_SEND_RESPONSE:
                ...
                {
                    _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                                 "[socket:%d] server handler sending response before last body chunk",
                                 _socket.getSocketDescriptor());
                }
...
ESFError AWSHttpServerSocket::skipTrailer(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(_state & SKIPPING_TRAILER);

    ESFError error = _transaction.getParser()->skipTrailer(_transaction.getIOBuffer());
...
ESFError AWSHttpServerSocket::formatResponseHeaders(ESFFlag *isRunning, ESFLogger *logger)
{
...
ESFError AWSHttpServerSocket::formatResponseBody(ESFFlag *isRunning, ESFLogger *logger)
{
    ESFError error;
    int availableSize = 0;
    int requestedSize = 0;
    ...
    while (isRunning->get())
    {
        ...
        if (0 > requestedSize)
        {
            ...
            {
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[socket:%d] server handler aborted connection will sending response body",
                             _socket.getSocketDescriptor());
            }
        ...
        _handler->fillResponseChunk(&_transaction,
...
ESFError AWSHttpServerSocket::flushBuffer(ESFFlag *isRunning, ESFLogger *logger)
{
    if (_logger->isLoggable(ESFLogger::Debug))
    {
        _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
...
bool AWSHttpServerSocket::sendResponse(ESFFlag *isRunning, ESFLogger *logger)
{
    if (0 == _transaction.getResponse()->getStatusCode())
    {
        ...
        {
            _logger->log(ESFLogger::Error, __FILE__, __LINE__,
                         "[socket:%d] server handler failed to build response, sending 500 Internal Server Error",
                         _socket.getSocketDescriptor());
        }
    ...
    // TODO strip Transfer-Encoding, Content-Length, & Connection headers from the response object
    // TODO add date header and any other stupid headers like that

    ESFError error = _transaction.getResponse()->addHeader((const unsigned char *) "Transfer-Encoding",
    ...
    {
        error = _transaction.getResponse()->addHeader((const unsigned char *) "Connection",
    ...
    return YieldAfterParsingRequest ? true : handleWritableEvent(isRunning, logger);
}

bool AWSHttpServerSocket::sendBadRequestResponse(ESFFlag *isRunning, ESFLogger *logger)
{
    _transaction.getResponse()->setStatusCode(400);
    _transaction.getResponse()->setReasonPhrase((const unsigned char *) "Bad Request");
    _transaction.getResponse()->setHasBody(false);

    return sendResponse(isRunning, logger);
}

bool AWSHttpServerSocket::sendInternalServerErrorResponse(ESFFlag *isRunning, ESFLogger *logger)
{
    // TODO reserve a static read-only internal server error response for out of memory conditions.

    _transaction.getResponse()->setStatusCode(500);
    _transaction.getResponse()->setReasonPhrase((const unsigned char *) "Internal Server Error");
    _transaction.getResponse()->setHasBody(false);

    ESFError error = _transaction.getResponse()->addHeader((const unsigned char *) "Content-Length",
    ...
    if (110 <= _transaction.getRequest()->getHttpVersion() &&
        (false == _transaction.getRequest()->getReuseConnection() || CloseAfterErrorResponse))
    {
        ESFError error = _transaction.getResponse()->addHeader((const unsigned char *) "Connection",
                                                                (const unsigned char *) "close",
                                                                _transaction.getAllocator());
