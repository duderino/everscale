...
AWSHttpClientSocketFactory::AWSHttpClientSocketFactory(AWSHttpClientCounters *clientCounters, ESFLogger *logger) :
    _logger(logger ? logger : ESFNullLogger::GetInstance()), _clientCounters(clientCounters), _allocator(), _map(true, &AddressComparator, &_allocator, // Will result in a leak if erase(), clear(), or insert() with a uniqueness violation
            ESFNullLock::Instance()), _embeddedList(), _mutex(), _cleanupHandler(this) {
}

ESFError AWSHttpClientSocketFactory::initialize() {
    return _allocator.initialize(ESF_WORD_ALIGN(sizeof(AWSHttpClientSocket)) * 1000, ESFSystemAllocator::GetInstance());
}
...
void AWSHttpClientSocketFactory::destroy() {
    ...
    for (ESFMapIterator iterator = _map.getMinimumIterator(); false == iterator.isNull(); iterator = iterator.getNext()) {
...
AWSHttpClientSocket *AWSHttpClientSocketFactory::create(AWSHttpConnectionPool *pool, AWSHttpClientTransaction *transaction) {
...

    {
        ...
        if (list) {
            ...
            if (socket) {
                if (_logger->isLoggable(ESFLogger::Debug)) {
                    ...
                    _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[pool] Reusing connection for %s:%d", buffer, transaction->getPeerAddress()->getPort());
                }
        ...
        if (_logger->isLoggable(ESFLogger::Debug)) {
            ...
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[pool] Creating new connection for %s:%d", buffer, transaction->getPeerAddress()->getPort());
        }
    ...
    if (0 == socket) {
        ...
        socket = new (&_allocator) AWSHttpClientSocket(pool, transaction, _clientCounters, &_cleanupHandler, _logger);
        ...
        if (0 == socket) {
            if (_logger->isLoggable(ESFLogger::Critical)) {
                ...
                _logger->log(ESFLogger::Critical, __FILE__, __LINE__, "[pool] Cannot allocate new connection for %s:%d", buffer,
...
void AWSHttpClientSocketFactory::release(AWSHttpClientSocket *socket) {
    ...
    if (false == socket->isConnected()) {
        if (_logger->isLoggable(ESFLogger::Debug)) {
            ...
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[pool] Not returning connection for %s:%d to pool", buffer, socket->getPeerAddress()->getPort());
        }
    ...
    if (list) {
        if (_logger->isLoggable(ESFLogger::Debug)) {
            ...
            _logger->log(ESFLogger::Debug, __FILE__, __LINE__, "[pool] Returning connection for %s:%d to pool", buffer, socket->getPeerAddress()->getPort());
        }
    ...
    if (0 == address) {
        if (_logger->isLoggable(ESFLogger::Warning)) {
            ...
            _logger->log(ESFLogger::Critical, __FILE__, __LINE__, "[pool] Cannot return connection for %s:%d to pool: bad alloc", buffer,
    ...
    if (ESF_SUCCESS != error) {
        ...
        if (_logger->isLoggable(ESFLogger::Warning)) {
            ...
            _logger->log(ESFLogger::Critical, __FILE__, __LINE__, "[pool] Cannot return connection for %s:%d to pool: %s", dottedIP,
                    socket->getPeerAddress()->getPort(), buffer);
        }
