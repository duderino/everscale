...
AWSHttpClientHistoricalCounters::AWSHttpClientHistoricalCounters(
    time_t windowSizeSec, ESFAllocator *allocator, ESFLogger *logger)
...
const AWSPerformanceCounter *AWSHttpClientHistoricalCounters::getSuccesses()
    const {
  return &_successes;
}
...
const AWSPerformanceCounter *AWSHttpClientHistoricalCounters::getFailures()
    const {
  return &_failures;
}
