...
AWSHttpClientSocket::AWSHttpClientSocket(AWSHttpConnectionPool *pool,
                                         ...
                                         AWSHttpClientCounters *counters,
...
ESFError AWSHttpClientSocket::reset(bool reused, AWSHttpConnectionPool *pool,
                                    AWSHttpClientTransaction *transaction) {
  if (!transaction) {
...
bool AWSHttpClientSocket::wantAccept() { return false; }

bool AWSHttpClientSocket::wantConnect() { return _state & CONNECTING; }
...
bool AWSHttpClientSocket::handleAcceptEvent(ESFFlag *isRunning,
                                            ESFLogger *logger) {
...
bool AWSHttpClientSocket::handleConnectEvent(ESFFlag *isRunning,
                                             ESFLogger *logger) {
  ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpClientSocket::handleReadableEvent(ESFFlag *isRunning,
                                              ESFLogger *logger) {
...
bool AWSHttpClientSocket::handleWritableEvent(ESFFlag *isRunning,
                                              ESFLogger *logger) {
...
bool AWSHttpClientSocket::handleErrorEvent(ESFError errorCode,
                                           ESFFlag *isRunning,
                                           ESFLogger *logger) {
  ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpClientSocket::handleEndOfFileEvent(ESFFlag *isRunning,
                                               ESFLogger *logger) {
...
bool AWSHttpClientSocket::handleIdleEvent(ESFFlag *isRunning,
                                          ESFLogger *logger) {
  ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
...
bool AWSHttpClientSocket::handleRemoveEvent(ESFFlag *isRunning,
                                            ESFLogger *logger) {
  ESF_ASSERT(!(HAS_BEEN_REMOVED & _state));
  ...
  if (_state & TRANSACTION_BEGIN) {
    ...
    _transaction->getHandler()->end(
        _transaction, AWSHttpClientHandler::AWS_HTTP_CLIENT_HANDLER_BEGIN);
  } else if (_state & CONNECTING) {
    _counters->getFailures()->addObservation(_transaction->getStartTime());

    _transaction->getHandler()->end(
        _transaction, AWSHttpClientHandler::AWS_HTTP_CLIENT_HANDLER_CONNECT);
    ...
    if (GetReuseConnections()) {
      const AWSHttpHeader *header = _transaction->getResponse()->getHeader(
    ...
    _transaction->getHandler()->end(
        _transaction, AWSHttpClientHandler::AWS_HTTP_CLIENT_HANDLER_END);
...
ESFError AWSHttpClientSocket::parseResponseHeaders(ESFFlag *isRunning,
                                                   ESFLogger *logger) {
  ...
  if (_logger->isLoggable(ESFLogger::Debug)) {
    ...
    for (AWSHttpHeader *header = (AWSHttpHeader *)_transaction->getResponse()
                                     ->getHeaders()
                                     ->getFirst();
...
ESFError AWSHttpClientSocket::parseResponseBody(ESFFlag *isRunning,
                                                ESFLogger *logger) {
...
ESFError AWSHttpClientSocket::formatRequestHeaders(ESFFlag *isRunning,
                                                   ESFLogger *logger) {
...
ESFError AWSHttpClientSocket::formatRequestBody(ESFFlag *isRunning,
                                                ESFLogger *logger) {
  ESFError error;
  int availableSize = 0;
  int requestedSize = 0;
...
ESFError AWSHttpClientSocket::flushBuffer(ESFFlag *isRunning,
                                          ESFLogger *logger) {
  if (_logger->isLoggable(ESFLogger::Debug)) {
    _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
