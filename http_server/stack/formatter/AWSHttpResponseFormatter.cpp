...
ESFError AWSHttpResponseFormatter::formatStartLine(ESFBuffer *outputBuffer, const AWSHttpMessage *message)
{
    ...
    if (AWS_FORMATTING_VERSION & _responseState)
    {
        ...
        AWSHttpUtil::Transition(&_responseState, outputBuffer, AWS_FORMATTING_VERSION, AWS_FORMATTING_STATUS_CODE);
    }
    ...
    if (AWS_FORMATTING_STATUS_CODE & _responseState)
    {
        ...
        AWSHttpUtil::Transition(&_responseState, outputBuffer, AWS_FORMATTING_STATUS_CODE, AWS_FORMATTING_REASON_PHRASE);
    }
    ...
    if (AWS_FORMATTING_REASON_PHRASE & _responseState)
    {
        ...
        return AWSHttpUtil::Transition(&_responseState, outputBuffer, AWS_FORMATTING_REASON_PHRASE, AWS_FORMAT_COMPLETE);
    }

    return ESF_INVALID_STATE;
}

ESFError AWSHttpResponseFormatter::formatStatusCode(ESFBuffer *outputBuffer, const AWSHttpResponse *response)
{
    // Status-Code    = 3DIGIT
    ...
    ESFError error = AWSHttpUtil::FormatInteger(outputBuffer, response->getStatusCode(), 10);
...
ESFError AWSHttpResponseFormatter::formatReasonPhrase(ESFBuffer *outputBuffer, const AWSHttpResponse *response)
{
