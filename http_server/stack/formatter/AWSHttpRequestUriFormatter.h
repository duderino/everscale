...
private:
    ...
    // "*"
    ESFError formatAsterisk(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);

    // abs_path      = "/"  path_segments
    // path_segments = segment *( "/" segment )
    // segment       = *pchar *( ";" param )
    // param         = *pchar
    ESFError formatAbsPath(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);

    // query         = *uric
    ESFError formatQuery(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);

    // fragment     = *uric
    ESFError formatFragment(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);

    // http_URL       = "http:" "//" host [ ":" port ] [ abs_path [ "?" query ]]
    // absoluteURI   = scheme ":" ( hier_part | opaque_part )
    // scheme        = alpha *( alpha | digit | "+" | "-" | "." )
    ESFError formatScheme(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);
    ...
    ESFError formatHost(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);

    // port          = *digit
    ESFError formatPort(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);
    ...
    ESFError formatNonHttpUri(ESFBuffer *outputBuffer, const AWSHttpRequestUri *requestUri);
