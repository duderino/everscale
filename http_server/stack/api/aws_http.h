 ...
 */
typedef aws_http_server_handler_result (*aws_http_server_handler_accept_connection)(const struct sockaddr_in *address);
 ...
 * @return a result code
 */
typedef aws_http_server_handler_result (*aws_http_server_handler_begin_transaction)(aws_http_transaction transaction);
 ...
 * @return a result code
 */
typedef aws_http_server_handler_result (*aws_http_server_handler_receive_request_headers)(aws_http_transaction transaction);
 ...
 * @return a result code
 */
typedef aws_http_server_handler_result (*aws_http_server_handler_receive_request_body)(aws_http_transaction transaction,
 ...
 */
typedef int (*aws_http_server_handler_reserve_response_chunk)(aws_http_transaction transaction);
 ...
 */
typedef void (*aws_http_server_handler_fill_response_chunk)(aws_http_transaction transaction,
                                                            unsigned char *chunk,
                                                            unsigned int chunkSize);
 ...
 */
typedef void (*aws_http_server_handler_end_transaction)(aws_http_transaction transaction,
                                                        aws_http_server_handler_state state);
...
typedef int (*aws_http_logger)(void *log_context, aws_http_log_level level, const char *file, int line, const char *format, va_list ap);
...
extern aws_http_stack aws_http_stack_create(aws_http_server *server,
                                            const aws_http_server_config *server_config,
                                            const aws_http_client_config *client_config,
...
extern void aws_http_stack_set_log_level(aws_http_stack stack, aws_http_log_level log_level);

typedef enum
{
    AWS_HTTP_CLIENT_HANDLER_CONTINUE = 0,       /**< Continue processing the request */
    AWS_HTTP_CLIENT_HANDLER_CLOSE = 1           /**< Immediately close the socket / abort the request */
} aws_http_client_handler_result;

/**
 * Request a buffer of up to n bytes to fill with request body data.  The stack will
 * subsequently call fillRequestChunk with the actual size of the buffer available.  The
 * actual size of the buffer may be less than the requested size.
 *
 * A good implementation will always return the known size of remaining body data.  This function
 * may be called multiple times before fillRequestChunk actually writes the data to the buffer
 * if there is insufficient space in the underlying tcp buffers.  Don't 'deduct' the amount
 * requested from the remaining amount until fillRequestChunk is called.
 *
 * @param transaction The http transaction - contains request and response objects, etc.
 * @return The buffer size requested.  Returning 0 ends the body.  Returning -1 or less immediately
 *   closes the connection
 */
typedef int (*aws_http_client_handler_reserve_request_chunk)(aws_http_transaction transaction);
     ...
     */
typedef void (*aws_http_client_handler_fill_request_chunk)(aws_http_transaction transaction,
                                                           unsigned char *chunk,
                                                           unsigned int chunkSize);
 ...
 * @return a result code
 */
typedef aws_http_client_handler_result (*aws_http_client_handler_receive_response_headers)(aws_http_transaction transaction);

/**
 * Incrementally process a response body.  This will be called 1+ times as the HTTP response
 * body is received.  The body is finished when a chunk_size of 0 is passed to the callback.  If
 * there is no body, this callback will still be called once with a chunk_size of 0.
 *
 * @param transaction The http transaction - contains request and response objects, etc.
 * @param chunk A buffer to drain
 * @param chunkSize The size of the buffer to drain, or 0 if the body is finished.
 * @return a result code
 */
typedef aws_http_client_handler_result (*aws_http_client_handler_receive_response_body)(aws_http_transaction transaction,
                                                                                        unsigned const char *chunk,
                                                                                        unsigned int chunkSize);
typedef enum
{
    AWS_HTTP_CLIENT_HANDLER_BEGIN = 0,                  /**< Transaction starting - nothing happend yet */
    ...
    AWS_HTTP_CLIENT_HANDLER_SEND_REQUEST_HEADERS = 3,   /**< Trying to send request headers */
    AWS_HTTP_CLIENT_HANDLER_SEND_REQUEST_BODY = 4,      /**< Trying to send request body */
    AWS_HTTP_CLIENT_HANDLER_RECV_RESPONSE_HEADERS = 5,  /**< Trying to receive/parse response headers */
    AWS_HTTP_CLIENT_HANDLER_RECV_RESPONSE_BODY = 6,     /**< Trying to receive response body */
    AWS_HTTP_CLIENT_HANDLER_END = 7                     /**< Response received / transaction completed */
 ...
 *
 * @param transaction The http transaction - contains request and response objects, etc
 * @param state The state at which the transaction ended.  AWS_HTTP_CLIENT_HANDLER_END means the transaction
 *  was successfully completed, any other state indicates error - reason will be in the server logs.
 */
typedef void (*aws_http_client_handler_end_transaction)(aws_http_transaction transaction, aws_http_client_handler_state state);
 ...
 * @return a new client transaction if successful, null otherwise
  */
extern aws_http_transaction aws_http_stack_create_client_transaction(aws_http_stack stack,
 ...
 * @return 0 if the transaction was successfully started, -1 otherwise.  If -1 is returned,
 *   errno will be set and the caller should cleanup the transaction with
 *   aws_http_stack_destroy_client_transaction
 */
extern int aws_http_stack_execute_client_transaction(aws_http_stack stack,
                                                     aws_http_transaction transaction);


/**
 * Cleanup the client transaction.  Note that this will not free any app-specific
 * context.  Call this only if executeClientTransaction doesn't return 0.
 *
 * @param stack The stack that created the transaction
 * @param transaction The transaction
 */
extern void aws_http_stack_destroy_transaction(aws_http_stack stack, aws_http_transaction);
extern void aws_http_stack_destroy_request(aws_http_stack stack, aws_http_request request);

extern const unsigned char *aws_http_header_get_field_name(const aws_http_header header);
extern int aws_http_header_set_field_name(aws_http_message message,
                                          aws_http_header header,
                                          const unsigned char *field_name);
extern const unsigned char *aws_http_header_get_field_value(const aws_http_header header);
extern int aws_http_header_set_field_value(aws_http_message message,
                                           aws_http_header header,
                                           const unsigned char *field_value);

extern aws_http_header aws_http_message_get_first_header(aws_http_message message);
extern aws_http_header aws_http_message_get_last_header(aws_http_message);
extern aws_http_header aws_http_message_find_first_header(aws_http_message, const unsigned char *field_name);
extern aws_http_header aws_http_message_find_last_header(aws_http_message, const unsigned char *field_name);
extern aws_http_header aws_http_message_get_next_header(aws_http_header header);
extern aws_http_header aws_http_message_get_previous_header(aws_http_header header);
...
extern int aws_http_message_set_version(aws_http_message message, float version);

extern void aws_http_message_set_has_body(aws_http_message message, int has_body);
...
extern const unsigned char *aws_http_request_get_method(const aws_http_request request);
extern int aws_http_request_set_method(aws_http_connection connection,
                                       aws_http_request request,
                                       const unsigned char *method);
extern aws_http_request_uri *aws_http_request_get_request_uri(aws_http_connection connection,
                                                              aws_http_request request);
...
extern int aws_http_response_get_status_code(aws_http_connection connection,
                                             aws_http_response response);
extern int aws_http_response_set_reason_phrase(aws_http_connection connection,
                                               aws_http_response response,
