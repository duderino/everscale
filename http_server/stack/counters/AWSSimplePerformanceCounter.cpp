...
AWSSimplePerformanceCounter::AWSSimplePerformanceCounter(const char *name) :
    AWSPerformanceCounter(), _startTime(0), _stopTime(0), _name(name), _avgLatencyMsec(0.0), _minLatencyMsec(-1.0), _maxLatencyMsec(0.0), _throughput(0UL),
            _lock() {
}

AWSSimplePerformanceCounter::AWSSimplePerformanceCounter(const char *name, time_t startTime, time_t stopTime) :
    AWSPerformanceCounter(), _startTime(startTime), _stopTime(stopTime), _name(name), _avgLatencyMsec(0.0), _minLatencyMsec(-1.0), _maxLatencyMsec(0.0),
            _throughput(0UL), _lock() {
}
...
void AWSSimplePerformanceCounter::addObservation(const struct timeval *start, const struct timeval *stop) {
...
void AWSSimplePerformanceCounter::printSummary(FILE *file) const {
    ...
    fprintf(file, "{\"name\": \"%s\", \"n\": %lu, \"avglatmillis\": %f, \"minlatmillis\": %f, \"maxlatmillis\": %f, \"start\": %lu, \"stop\": %lu}",
            _name, throughput, avgLatencyMsec, minLatencyMsec, maxLatencyMsec, _startTime, _stopTime);
}
