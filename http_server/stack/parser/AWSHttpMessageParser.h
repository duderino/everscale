...
public:
     ...
     */
    AWSHttpMessageParser(ESFBuffer *workingBuffer, ESFDiscardAllocator *allocator);
    ...
    /**
     * Parse the body.  Caller should keep calling this mehtod on a given input buffer
     * until it returns an error, ESF_AGAIN (buffer needs to be compacted and filled),
     * or the number of bytes to be read is 0 (EOF)
     *
     ...
     * @param chunkSize If ESF_SUCCESS is returned, this will be set to the number of
     *  bytes that can be read after the starting position.  If 0, then there is no
     *  more data left in the body.
     * @return ESF_SUCCESS if successful, ESF_AGAIN if the buffer needs to be
     *  compacted and filled, another error code otherwise.
     */
    ESFError parseBody(ESFBuffer *inputBuffer, int *startingPosition, int *chunkSize);
     ...
     * @return ESF_SUCCESS if successful, ESF_AGAIN if the buffer needs to be compacted
     *  and filled, another error code otherwise.
     */
    ESFError skipTrailer(ESFBuffer *inputBuffer);

protected:

    virtual ESFError parseStartLine(ESFBuffer *inputBuffer, AWSHttpMessage *message) = 0;

    virtual bool isBodyNotAllowed(AWSHttpMessage *message) = 0;

    // HTTP-Version   = "HTTP" "/" 1*DIGIT "." 1*DIGIT
    ESFError parseVersion(ESFBuffer *inputBuffer, AWSHttpMessage *message, bool clientMode);
...
private:
    ...
    //                  trailer
    //                  CRLF
    ESFError parseChunkedBody(ESFBuffer *inputBuffer, int *startingPosition, int *chunkSize);
    ...
    ESFError parseChunkData(ESFBuffer *inputBuffer, int *startingPosition, int *chunkSize);

    // chunk          = ... CRLF
    ESFError parseEndChunk(ESFBuffer *inputBuffer);

    ESFError parseMultipartBody(ESFBuffer *inputBuffer, int *startingPosition, int *chunkSize);

    ESFError parseUnencodedBody(ESFBuffer *inputBuffer, int *startingPosition, int *chunkSize);
