...
class AWSHttpMessage {
 public:
  ...
  inline ESFEmbeddedList *getHeaders() { return &_headers; }

  inline const ESFEmbeddedList *getHeaders() const { return &_headers; }

  const AWSHttpHeader *getHeader(unsigned const char *fieldName) const;

  ESFError addHeader(unsigned const char *fieldName,
                     unsigned const char *fieldValue, ESFAllocator *allocator);

  ESFError addHeader(ESFAllocator *allocator, unsigned const char *fieldName,
  ...
  inline bool hasBody() const { return _flags & AWS_HTTP_MESSAGE_HAS_BODY; }
