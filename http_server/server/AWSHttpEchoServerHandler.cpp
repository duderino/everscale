...
AWSHttpServerHandler::Result AWSHttpEchoServerHandler::acceptConnection(ESFSocketAddress *address)
{
    if (_logger->isLoggable(ESFLogger::Debug))
    {
        ...
        _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                     "[handler] Accepted new connection: %s/%u",
                     dottedIP, address->getPort());
    }

    return AWS_HTTP_SERVER_HANDLER_CONTINUE;
}

AWSHttpServerHandler::Result AWSHttpEchoServerHandler::begin(AWSHttpTransaction *transaction)
{
    ...
    if (_logger->isLoggable(ESFLogger::Debug))
    {
        ...
        _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                     "[handler] Beginning new transaction: %s/%u",
                     dottedIP, transaction->getPeerAddress()->getPort());
    }

    AWSHttpEchoServerContext *context = new(allocator) AWSHttpEchoServerContext();
...
AWSHttpServerHandler::Result AWSHttpEchoServerHandler::receiveRequestHeaders(AWSHttpTransaction *transaction)
{
    ESF_ASSERT(transaction);
    ...
    if (_logger->isLoggable(ESFLogger::Debug))
    {
        ...
        switch (transaction->getRequest()->getRequestUri()->getType())
        {
            ...
            case AWSHttpRequestUri::AWS_URI_HTTPS:
                ...
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[handler] Scheme: %s",
                             AWSHttpRequestUri::AWS_URI_HTTP == requestUri->getType() ? "http" : "https");
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[handler] Host: %s",
                             0 == requestUri->getHost() ? "none" : (const char *) requestUri->getHost());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[handler] Port: %d",
                             requestUri->getPort());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                ...
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[handler] Query: %s",
                             requestUri->getQuery() ? "none" : (const char *) requestUri->getQuery());
                _logger->log(ESFLogger::Debug, __FILE__, __LINE__,
                             "[handler] Fragment: %s",
                             requestUri->getFragment() ? "none" : (const char *) requestUri->getFragment());
        ...
        for (AWSHttpHeader *header = (AWSHttpHeader *) request->getHeaders()->getFirst();
             header;
...
AWSHttpServerHandler::Result AWSHttpEchoServerHandler::receiveRequestBody(AWSHttpTransaction *transaction,
...
int AWSHttpEchoServerHandler::reserveResponseChunk(AWSHttpTransaction *transaction)
{
    AWSHttpEchoServerContext *context = (AWSHttpEchoServerContext *) transaction->getApplicationContext();
...
void AWSHttpEchoServerHandler::fillResponseChunk(AWSHttpTransaction *transaction,
                                                 unsigned char *chunk,
                                                 unsigned int chunkSize)
{
    ESF_ASSERT(transaction);
