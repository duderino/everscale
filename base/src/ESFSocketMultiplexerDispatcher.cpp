...
ESFSocketMultiplexerDispatcher::ESFSocketMultiplexerDispatcher(
    ESFUInt16 maximumSockets, ESFUInt16 multiplexerCount,
    ESFSocketMultiplexerFactory *factory, ESFAllocator *allocator,
    const char *name, ESFLogger *logger)
...
void ESFSocketMultiplexerDispatcher::stop() {
  ...
  if (_logger->isLoggable(ESFLogger::Notice)) {
    _logger->log(ESFLogger::Notice, __FILE__, __LINE__,
                 "[%s:dispatcher] stopped", _name);
  }
}

ESFError ESFSocketMultiplexerDispatcher::addMultiplexedSocket(
...
ESFError ESFSocketMultiplexerDispatcher::addMultiplexedSocket(
    int multiplexerIndex, ESFMultiplexedSocket *multiplexedSocket) {
...
ESFError ESFSocketMultiplexerDispatcher::createMultiplexers() {
  _multiplexers = (ESFSocketMultiplexer **)_allocator->allocate(
      _multiplexerCount * sizeof(ESFSocketMultiplexer *));
