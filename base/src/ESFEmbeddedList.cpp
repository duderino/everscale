 ...
 * (http://sourceforge.net/projects/sparrowhawk/).v
 *
 *    $Author: blattj $
...
ESFEmbeddedList::ESFEmbeddedList() : _head(0), _tail(0) {}
...
void ESFEmbeddedList::clear(bool cleanup) {
  ...
  for (ESFEmbeddedListElement *element = removeFirst(); element;
       element = removeFirst()) {
...
int ESFEmbeddedList::length() const {
  ...
  for (ESFEmbeddedListElement *current = _head; current;
