...
ESFConnectedTCPSocket::ESFConnectedTCPSocket(bool isBlocking) :
    ESFTCPSocket(isBlocking), _isConnected(false), _listenerAddress(), _peerAddress() {
}

ESFConnectedTCPSocket::ESFConnectedTCPSocket(const ESFSocketAddress &peer, bool isBlocking) :
    ESFTCPSocket(isBlocking), _isConnected(false), _listenerAddress(), _peerAddress(peer) {
}

ESFConnectedTCPSocket::ESFConnectedTCPSocket(AcceptData *acceptData) :
    ESFTCPSocket(acceptData), _isConnected(true), _listenerAddress(acceptData->_listeningAddress), _peerAddress(
            acceptData->_peerAddress) {
}
...
ESFConnectedTCPSocket::getPeerAddress() const {
    return _peerAddress;
}

const ESFSocketAddress &
ESFConnectedTCPSocket::getListenerAddress() const {
...
#error "socket or equivalent is required"
    ...
    if (SOCKET_ERROR == ::connect(_sockFd, (sockaddr *) _peerAddress.getAddress(), sizeof(ESFSocketAddress::Address))) {
...
ESFSSize ESFConnectedTCPSocket::receive(ESFBuffer *buffer) {
    ESFSSize size = receive((char *) buffer->getBuffer() + buffer->getWritePosition(), buffer->getWritable());
...
ESFSSize ESFConnectedTCPSocket::send(ESFBuffer *buffer) {
    ESFSize size = send((char *) buffer->getBuffer() + buffer->getReadPosition(), buffer->getReadable());
