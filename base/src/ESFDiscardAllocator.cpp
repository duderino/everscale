...
ESFDiscardAllocator::~ESFDiscardAllocator() { destroy(); }
...
ESFUWord ESFDiscardAllocator::getOverhead() { return 0; }

ESFError ESFDiscardAllocator::initialize() { return ESF_SUCCESS; }
...
ESFError ESFDiscardAllocator::isInitialized() { return ESF_SUCCESS; }
...
ESFDiscardAllocator::Chunk *ESFDiscardAllocator::allocateChunk(int chunkSize) {
  Chunk *tmp =
      (Chunk *)_source->allocate(ESF_WORD_ALIGN(sizeof(Chunk)) + chunkSize);
