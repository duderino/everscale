...
ESFConsoleLogger *ESFConsoleLogger::Instance() { return &_Instance; }

ESFConsoleLogger::ESFConsoleLogger() : _severity(None) {}
...
void ESFConsoleLogger::setSeverity(Severity severity) { _severity = severity; }

ESFError ESFConsoleLogger::log(Severity severity, const char *file, int line,
                               const char *format, ...) {
#ifdef ALLOW_CONSOLE_LOGGING
  if (!file || !format) {
...
#error "va_end or equivalent is required"
  ...
  snprintf(buffer, sizeof(buffer), "%s,%s,%d," THREAD_ID_FORMAT " ",
           CodeToString(severity), file, line, ESFThread::GetThreadId());
#else
