...
void *ESFMapNode::operator new(size_t size, ESFAllocator *allocator) {
...
ESFError ESFMap::insert(const void *key, void *value) {
  ...
  ESFMapNode *node = new (_allocator)
      ESFMapNode(&_sentinel, &_sentinel, &_sentinel, true, key, value);
...
ESFMapNode *ESFMap::findNode(ESFMapNode *x, const void *k) {
  //
...
ESFMapNode *ESFMap::findMinimum(ESFMapNode *x) {
  //
...
ESFMapNode *ESFMap::findMaximum(ESFMapNode *x) {
  //
...
ESFMapNode *ESFMap::findSuccessor(ESFMapNode *x) {
  //
...
ESFMapNode *ESFMap::findPredecessor(ESFMapNode *x) {
  //
  //  See Tree-Successor in "Introduction to Algorithms", Cormen, Leiserson,
  //  Rivest, p. 249.
  //
...
ESFUInt32 ESFMap::getSize() const { return _size; }

bool ESFMap::isEmpty() const { return 0 == _size; }
...
ESFSize ESFMap::GetAllocationSize() { return sizeof(ESFMapNode); }

ESFMapIterator::ESFMapIterator() : _node(0) {}

ESFMapIterator::ESFMapIterator(ESFMapNode *node) : _node(node) {}
