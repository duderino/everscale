...
public:
    ...
    /** Constructor.
     *
     * @param name The name of the multiplexer to be used in log messages.  Caller is responsible
     *  for the strings memory - use a string literal if possible.
     ...
     */
    ESFEpollMultiplexer(const char *name, int maxSockets, ESFLogger *logger, ESFAllocator *allocator);
     ...
     */
    virtual ESFError addMultiplexedSocket(ESFMultiplexedSocket *multiplexedSocket);
...
private:
     ...
     */
    ESFError updateMultiplexedSocket(ESFFlag *isRunning, ESFMultiplexedSocket *multiplexedSocket);

    /** Remove a multiplexed socket form the socket multiplexer
     *
     * @param multiplexedSocket The multiplexed socket to remove
     */
    ESFError removeMultiplexedSocket(ESFFlag *isRunning, ESFMultiplexedSocket *multiplexedSocket, bool removeFromList =
            true);
