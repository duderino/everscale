 ...
 *  @brief BuddyAllocatorTest is the unit test for ESFBuddyAllocator.
 */
...
ESTF_OBJECT_PTR(ESFBuddyAllocatorTest, Component)
...
bool BuddyAllocatorTest::run(ResultCollector *collector) {
  //
      ...
      if (0 == allocations[j].size && 1 == _rand.generateRandom(1, 4)) {
        ...
        if (Debug) {
          cerr << "Allocating block of size " << allocations[j].size
               << " at time " << i
               << " with lifetime: " << allocations[j].lifetime << endl;
        }
        ...
        if (!allocations[j].data) {
          if (Debug) {
            cerr << "Failed to allocate block of size " << allocations[j].size
                 << " at time " << i
                 << " with lifetime: " << allocations[j].lifetime << endl;
          }
...
int main() {
  BuddyAllocatorTestPtr buddyAllocatorTest = new BuddyAllocatorTest();
