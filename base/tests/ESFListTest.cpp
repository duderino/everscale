...
ESFListTest::ESFListTest()
    : _records(0),
      _rand(),
      _list(ESFSystemAllocator::GetInstance(), &_Lock),
      _stlList() {}
...
bool ESFListTest::run(ESTFResultCollector *collector) {
  ...
  for (int k = 0; k < 3; ++k) {
    ...
    for (int i = 0; i < _Iterations; ++i) {
      for (int j = 0; j < _Records; ++j) {
        if (!_records[j]._value && 1 == _rand.generateRandom(1, 200)) {
          ...
          if (1 == _rand.generateRandom(1, 2)) {
            ...
            ESTF_ASSERT(collector,
                        ESF_SUCCESS == _list.pushBack(_records[j]._value));
            ...
            ESTF_ASSERT(collector,
                        ESF_SUCCESS == _list.pushFront(_records[j]._value));
        ...
        if (_records[j]._value && i == _records[j]._lifetime) {
          ...
          if (_records[j]._useIterator) {
            ESTF_ASSERT(collector,
                        ESF_SUCCESS == _list.erase(&_records[j]._iterator));
            ...
            ESTF_ASSERT(collector, ESF_SUCCESS == _list.erase(&iterator));

            ESTF_ASSERT(collector, iterator.isNull());
          }

          if (Debug) {
            cerr << "value[" << j << "]: " << _records[j]._value << endl;
          }
    ...
    for (value = (char *)_list.getFront(); !_list.isEmpty();
      ...
      if (Debug) {
        cerr << "Deleted: " << value << " (List size: " << _list.getSize()
...
bool ESFListTest::setup() { return true; }

bool ESFListTest::tearDown() { return true; }
...
char *ESFListTest::generateValue(int i, int j) {
