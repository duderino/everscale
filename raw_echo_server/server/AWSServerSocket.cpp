...
ESFBufferPool AWSServerSocket::_BufferPool(1024, 900, ESFSystemAllocator::GetInstance());
...
bool AWSServerSocket::isIdle()  // todo pass in current time to reduce number of syscalls
{
    ESF_ASSERT(false == _hasBeenRemoved);
...
bool AWSServerSocket::handleConnectEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(false == _hasBeenRemoved);
...
bool AWSServerSocket::handleErrorEvent(ESFError errorCode, ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(false == _hasBeenRemoved);
...
bool AWSServerSocket::handleEndOfFileEvent(ESFFlag *isRunning, ESFLogger *logger)
{
    ESF_ASSERT(false == _hasBeenRemoved);
...
const char *AWSServerSocket::getName() const
{
    return "AWSServerSocket";
}
