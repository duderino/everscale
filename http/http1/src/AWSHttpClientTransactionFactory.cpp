...
AWSHttpClientTransaction *AWSHttpClientTransactionFactory::create(
  ...
  if (0 == transaction) {
    transaction = new (&_allocator)
        AWSHttpClientTransaction(clientHandler, &_cleanupHandler);
...
void AWSHttpClientTransactionFactory::release(
    AWSHttpClientTransaction *transaction) {
  if (!transaction) {
    return;
  }
...
AWSHttpClientTransactionFactory::CleanupHandler::CleanupHandler(
    AWSHttpClientTransactionFactory *factory)
    : ESFCleanupHandler(), _factory(factory) {}

AWSHttpClientTransactionFactory::CleanupHandler::~CleanupHandler() {}

void AWSHttpClientTransactionFactory::CleanupHandler::destroy(
    ESFObject *object) {
