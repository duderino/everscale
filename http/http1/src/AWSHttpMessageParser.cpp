...
AWSHttpMessageParser::AWSHttpMessageParser(ESFBuffer *workingBuffer,
...
ESFError AWSHttpMessageParser::parseHeaders(ESFBuffer *inputBuffer,
...
ESFError AWSHttpMessageParser::parseFieldName(ESFBuffer *inputBuffer,
                                              AWSHttpMessage *message) {
  // field-name     = token
...
ESFError AWSHttpMessageParser::parseFieldValue(ESFBuffer *inputBuffer,
                                               AWSHttpMessage *message) {
      ...
      switch (error) {
        case ESF_SUCCESS:
...

          {
            ...
            AWSHttpHeader *header =
                (AWSHttpHeader *)message->getHeaders()->getLast();
...
ESFError AWSHttpMessageParser::parseBody(ESFBuffer *inputBuffer,
...
ESFError AWSHttpMessageParser::parseUnencodedBody(ESFBuffer *inputBuffer,
                                                  int *startingPosition,
                                                  int *chunkSize) {
  ...
  ESF_ASSERT((AWS_PARSING_UNENCODED_BODY | AWS_PARSING_BODY_UNTIL_CLOSE) &
             _state);
...
ESFError AWSHttpMessageParser::parseChunkedBody(ESFBuffer *inputBuffer,
                                                int *startingPosition,
                                                int *chunkSize) {
  // Chunked-Body   = *chunk
  //                  last-chunk
...
ESFError AWSHttpMessageParser::parseChunkData(ESFBuffer *inputBuffer,
                                              int *startingPosition,
                                              int *chunkSize) {
  // chunk-data     = chunk-size(OCTET)
...
ESFError AWSHttpMessageParser::parseMultipartBody(ESFBuffer *inputBuffer,
                                                  int *startingPosition,
                                                  int *chunkSize) {
  //
...
ESFError AWSHttpMessageParser::postParse(AWSHttpMessage *message) {
  ...
  for (AWSHttpHeader *header =
           (AWSHttpHeader *)message->getHeaders()->getFirst();
    ...
    if (0 ==
      ...
      error =
          AWSHttpUtil::ParseInteger(header->getFieldValue(), &contentLength);
...
ESFError AWSHttpMessageParser::parseVersion(ESFBuffer *inputBuffer,
                                            AWSHttpMessage *message,
                                            bool clientMode) {
  ...
  if (AWSHttpUtil::IsMatch(inputBuffer, (const unsigned char *)"HTTP/1.1")) {
    ...
    version = 110;
  } else if (AWSHttpUtil::IsMatch(inputBuffer,
