...
class AWSHistoricalPerformanceCounter : public AWSPerformanceCounter {
 public:
   ...
   */
  AWSHistoricalPerformanceCounter(const char *name, time_t windowSizeSec,
                                  ESFAllocator *allocator, ESFLogger *logger);
  ...
  /**
   * Get a list of AWSSimplePerformanceCounters.  There will be one counter in
   * the list per window that actually had observations.
   *
   * @return The list of AWSSimplePerformanceCounters.
   */
  inline const ESFEmbeddedList *getCounters() const { return &_list; }

  virtual void printSummary(FILE *file) const;

 private:
  // Disabled
  AWSHistoricalPerformanceCounter(
      const AWSHistoricalPerformanceCounter &counter);
  AWSHistoricalPerformanceCounter *operator=(
      const AWSHistoricalPerformanceCounter &counter);
