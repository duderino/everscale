...
class HttpRoutingProxyHandler : public HttpProxyHandler {
 public:
  ...
  virtual ESB::Error beginTransaction(HttpMultiplexer &multiplexer, HttpServerStream &stream);
  ...
  virtual void endTransaction(HttpMultiplexer &multiplexer, HttpServerStream &stream, HttpServerHandler::State state);
  ...
  virtual void endTransaction(HttpMultiplexer &multiplexer, HttpClientStream &stream, HttpClientHandler::State state);
 ...
 private:
  ...
  ESB::Error sendResponse(HttpServerStream &stream, int statusCode, const char *reasonPhrase);
