...
static const int MaxSockets =
    ESFSocketMultiplexerDispatcher::GetMaximumSockets();

AWSHttpStack::AWSHttpStack(AWSHttpServerHandler *serverHandler,
                           AWSHttpResolver *resolver, int port, int threads,
                           AWSHttpClientCounters *clientCounters,
                           AWSHttpServerCounters *serverCounters,
                           ESFLogger *logger)
...
AWSHttpStack::AWSHttpStack(AWSHttpResolver *resolver, int threads,
                           AWSHttpClientCounters *clientCounters,
                           ESFLogger *logger)
    : _port(-1),
      _threads(0 >= threads ? 1 : threads),
...
ESFError AWSHttpStack::start() {
  ...
  for (int i = 0; i < _threads; ++i) {
    socket = new (&_rootAllocator) AWSHttpListeningSocket(
...
AWSHttpClientTransaction *AWSHttpStack::createClientTransaction(
...
ESFError AWSHttpStack::executeClientTransaction(
    AWSHttpClientTransaction *transaction) {
  if (!transaction) {
...
void AWSHttpStack::destroyClientTransaction(
    AWSHttpClientTransaction *transaction) {
  if (AWS_HTTP_STACK_IS_DESTROYED == _state) {
    return;
  }
